...
/**
 * Controlador que permite el enlace entre el modelo y las vistas de planes de estudio externos
 * @author Luis Dominguez, Leslie Navez
 * @since 2021
 */
class PlanEstudiosExternoController {
    /**
    ...
    /**
     * Inyección de PlanEstudiosExternoService que contiene la lógica de administración de planes de estudio externos
     */
     ...
     */
    def registrar(){
        def resultado = planEstudiosExternoService.registrar(params)
    ...
    /**
     * Obtiene los planes externos activos para mostrarlos en la vista 'listar'
     ...
     */
    def listar(){
        def resultado = planEstudiosExternoService.listar(params)
        def instituciones = institucionExternaService.obtenerActivos(params)
        def carreras = carreraExternaService.obtenerActivos(params)
        ...
        [
            ...
            numeroAsignaturas: resultado.datos.numeroAsignaturas,
            parametros: params
        ]
    }

    /**
     * Obtiene un plan externo para mostrarlo en la vista 'consultar'
     * @param id (Requerido)
     * Identificador del plan
     */
    def consultar(){
        def resultado = planEstudiosExternoService.consultar(params)
        flash.estatus = resultado.estatus
        flash.mensaje = resultado.mensaje
    ...
    /**
     * Modifica los datos de un plan externo
     * @param id (Requerido)
     * Identificador del plan
     * @param nombre (Requerido)
     * @param carreraId (Requerido)
     ...
     */
    def modificar(){
        def resultado = planEstudiosExternoService.modificar(params)
        flash.estatus = resultado.estatus
        flash.mensaje = resultado.mensaje
    ...
    /**
     * Realiza una baja lógica de un plan externo
     * @param id (Requerido)
     * Identificador del plan
     */
    def eliminar(){
        def resultado = planEstudiosExternoService.eliminar(params)
        flash.estatus = resultado.estatus
        flash.mensaje = resultado.mensaje
