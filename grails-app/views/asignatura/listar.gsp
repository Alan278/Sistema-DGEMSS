...
<html>
    ...
    <body>
        ...
        <div class="container">
            <form action="/asignatura/listar" class="mt-4" id="form-busqueda">
            ...
            <table class="table table-striped">
                <thead>
                    <tr>
                        ...
                        <th class="align-middle">Tipo formación</th>
                        <th></th>
                    </tr>
                </thead>
                <tBody>
                    <g:each status="i" in="${asignaturas}" var="asignatura">
                            <tr>
                                <td class="align-middle text-center">${asignatura?.clave}</td>
                                <td class="align-middle text-center">${asignatura?.periodo}</td>
                                <td class="align-middle"><a onclick="mostrarProceso();" href="/asignatura/consultar/${asignatura?.id}" class="alert-link" data-tooltip="tooltip" data-placement="top" title="Consultar Asignatura">${asignatura?.nombre}</a></td>
                                <td class="align-middle">
                                    ${asignatura?.planEstudios?.carrera?.institucion?.nombre}
                                </td>
                                <td class="align-middle">${asignatura?.formacion?.nombre}</td>
                                <g:if test="${!asignatura.planEstudios.numCertificados}">
                                    <td class="align-middle" align="end">
                                            <a onclick="mostrarProceso();" href="/asignatura/modificacion/${asignatura?.id}" class="btn btn-outline-primary btn-sm" data-tooltip="tooltip" data-placement="top" title="Editar Registro">
                                                <i class="fa fa-pencil"></i>
                                            </a>
                                            <button type="button" class="btn btn-outline-danger btn-sm" data-id="${asignatura?.id}" data-toggle="modal" data-target="#modalEliminacion_${asignatura?.id}" data-tooltip="tooltip" data-placement="top" title="Eliminar Registro">
        ...
        <g:render template="/layouts/pagination" model="${[linkUri: "asignatura/listar", linkParams: [search: params.search?:"", institucionId: params.institucionId?:"", carreraId: params.carreraId?:"", planEstudiosId: params.planEstudiosId?:""], count: conteo, max: params.max, offset: params.offset]}"/>
