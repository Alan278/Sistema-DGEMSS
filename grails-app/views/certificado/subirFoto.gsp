    ...
    <body>
        ...
        <div class="container mt-3 mb-5">
            <div class="row align-items-end">
                <div class="col-md-6">
                    <div class="card" >
                        ...
                        <div class="card-body py-2 px-3">
                            <div class="card-text">
                                <p class="m-0"> <b> Institución: </b>${persona?.alumnos[0]?.cicloEscolar?.planEstudios?.carrera?.institucion?.nombre} </p>
                                <p class="m-0"> <b> Carrera: </b> ${persona?.alumnos[0]?.cicloEscolar?.planEstudios?.carrera?.nombre}</p>
                ...
                <div class="col-md-4">
                    <p class="m-0"> <b> Libro: </b> ${params.libro ? params.libro : (persona?.certificados[0]?.libro)} </p>
                    <p class="m-0"> <b> Foja: </b> ${params.foja ? params.foja : (persona?.certificados[0]?.foja)}</p>
                    <p class="m-0"> <b> Municipio: </b> ${params.municipio ? params.municipio : (persona?.certificados[0]?.municipio)}</p>
                    <p class="m-0"> <b> Número: </b> ${params.numero ? params.numero : (persona?.certificados[0]?.numero)}</p>
                    <p class="m-0"> <b> Fecha de expedición: </b> ${params.fechaExpedicion ? params.fechaExpedicion : (persona?.certificados[0]?.fechaExpedicion)}</p>
                </div>
            </div>
        </div>
            ...
            <div class="row justify-content-center">
                <div class="col-md-6">
                    <form id="formulario" action="/certificado/registrar/${persona?.id}" method="POST" class="needs-validation" novalidate>
                        ...
                        <div class="row justify-content-center mt-5">
                            <p>
                                <a onclick="mostrarProceso();" href="certificado/listarAlumnos" class="btn btn-secondary">Cancelar</a>
                                <a onclick="mostrarProceso();" href="certificado/registro/${persona?.id}?libro=${params.libro}&foja=${params.foja}&numero=${params.numero}&municipio=${params.municipio}&fechaExpedicion=${params.fechaExpedicion}" class="btn btn-secondary">Volver</a>
