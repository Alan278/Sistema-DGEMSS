...
/**
 * Servicio que permite la administración de tramites realizados
...
class CertificacionService {
    ...
    def registrar(params){
        ...
        def datosBitacora = [
            clase: "CertificacionService",
            metodo: "registrar",
            nombre: "Registro de tramite de certificación",
        ...
        def resultadoOperacion = pagoService.consultarPago(params, tipoTramiteService.CERTIFICACION)
        ...
        if(!params.certificados){
            resultado.mensaje = 'Los certificados son un dato requerido'
            return resultado
        }
        ...
        def uma = Uma.get(1)
        def tipoTramite = tipoTramiteService.obtener(tipoTramiteService.CERTIFICACION)
        def precioCertificado = tipoTramite.costoUmas * uma.valor

        def numCertificadosPagados = Math.floor(pago.importe / precioCertificado)

        def certificadosId = []
        try{

            def jsonSlurper = new JsonSlurper()
            certificadosId = jsonSlurper.parseText(params.certificados)
            ...
            if(certificadosId.size > numCertificadosPagados){
                resultado.mensaje = "Seleccione ${numCertificadosPagados} certificados"
                return resultado
            }

            for(certificadoId in certificadosId){
                def certificado = Certificado.get(certificadoId)
                if(!certificado){
                    resultado.mensaje = "No se encontró el certificado"
                    return resultado
                }
                if(!certificado.activo){
                    resultado.mensaje = "El certificado no se encuentra activo"
                    return resultado
                }
                if(certificado.tramite != null){
                    resultado.mensaje = "El certificado ya cuenta con un tramite asignado"
                    return resultado
                }
                certificado.alumno.ciclosEscolares.each{ ciclo ->
            ...
            resultado.mensaje = "Error en formato de certificados ${ex}"
            return resultado
        }
		...
		certificadosId.each{ certificadoId ->
			def certificado = Certificado.get(certificadoId)
			certificado.tramite = tramite
			certificado.estatusCertificado = EstatusCertificado.get(certificadoService.EN_REVISION)
            certificado.save()
		}
        ...
        Pago.withTransaction {status ->
            if(tramite.pago.save(flush: true)){
                if(tramite.save(flush: true)){
                    resultado.estatus = true
                    bitacoraService.registrar([clase: "CertificacionService", metodo:"modificar", nombre:"Modificacion de tramite de certificación", descripcion:"Se modifica el tramite", estatus: "EXITOSO"])
                }else{
                    status.setRollbackOnly()
                    tramite.errors.allErrors.each {
                        resultado.mensaje = messageSource.getMessage(it, null)
                    }
                    bitacoraService.registrar([clase: "CertificacionService", metodo: "modificar", nombre:"Modificacion de tramite de certificación", descripcion: "Se modifica el tramite", estatus:"ERROR"])
                }
            }
        }
    ...
    def eliminar(params){
        ...
        if(!tramite.save(flush:true)){
            ...
            bitacoraService.registrar([clase:"CertificacionService", metodo: "eliminar", nombre:"Eliminación del tramite", descripcion:"Se elimina el tramite", estatus:"EXITOSO"])
            return resultado
        }
        bitacoraService([clase:"CertificacionService", metodo:"eliminar", nombre:"Eliminación del tramite", descripcion:"Se elimina el tramite", estatus:"ERROR"])
