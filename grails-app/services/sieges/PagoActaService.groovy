...
class PagoActaService {
    ...
    def registrar(params){
        ...
        def resultadoOperacion = pagoService.consultarPago(params, tipoTramiteService.ACTA)
        ...
        if(!params.actas){
            resultado.mensaje = 'Las actas son un dato requerido'
            return resultado
        }
        ...
        def uma = Uma.get(1)
        def tipoTramite = tipoTramiteService.obtener(tipoTramiteService.ACTA)
        def precioActa = tipoTramite.costoUmas * uma.valor

        def numActasPagados = Math.floor(pago.importe / precioActa)

        def actasId = []
        try{

            def jsonSlurper = new JsonSlurper()
            actasId = jsonSlurper.parseText(params.actas)

            if(!actasId){
                resultado.mensaje = 'Los acta son un dato requerido'
                return resultado
            }

            if(actasId.size > numActasPagados){
                resultado.mensaje = "Seleccione ${numActasPagados} acta"
                return resultado
            }

            for(actaId in actasId){
                def acta = ActaProfesional.get(actaId)
                if(!acta){
                    resultado.mensaje = "No se encontró el acta"
                    return resultado
                }
                if(!acta.activo){
                    resultado.mensaje = "El acta no se encuentra activo"
                    return resultado
                }
                if(acta.tramite != null){
                    resultado.mensaje = "El acta ya cuenta con un tramite asignado"
                    return resultado
                }
                acta.alumno.ciclosEscolares.each{ ciclo ->
		...
		actasId.each{ actaId ->
			def acta = ActaProfesional.get(actaId)
			acta.tramite = tramite
			acta.estatusActa = EstatusActa.get(actaService.EN_REVISION)
            acta.save()
		}
        ...
        Pago.withTransaction {status ->
            if(tramite.pago.save(flush: true)){
                if(tramite.save(flush: true)){
                    resultado.estatus = true
                    bitacoraService.registrar([clase: "PagoActaService", metodo:"modificar", nombre:"Modificacion de tramite de certificación", descripcion:"Se modifica el tramite", estatus: "EXITOSO"])
                }else{
                    status.setRollbackOnly()
                    tramite.errors.allErrors.each {
                        resultado.mensaje = messageSource.getMessage(it, null)
                    }
                    bitacoraService.registrar([clase: "PagoActaService", metodo: "modificar", nombre:"Modificacion de tramite de certificación", descripcion: "Se modifica el tramite", estatus:"ERROR"])
                }
            }
        }
    ...
    def eliminar(params){
        ...
        if(!tramite.save(flush:true)){
            ...
            bitacoraService.registrar([clase:"PagoActaService", metodo: "eliminar", nombre:"Eliminación del tramite", descripcion:"Se elimina el tramite", estatus:"EXITOSO"])
            return resultado
        }
        bitacoraService([clase:"PagoActaService", metodo:"eliminar", nombre:"Eliminación del tramite", descripcion:"Se elimina el tramite", estatus:"ERROR"])
