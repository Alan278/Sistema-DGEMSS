...
/**
 * Servicio que permite la administración de estatus de certificados
 * @author Luis Dominguez, Leslie Navez
 * @since 2021
 */
@Transactional
class EstatusCertificadoService {
	/**
	...
	def listar(params){
		...
		def estatus = EstatusCertificado.createCriteria().list(params,criteria)
	...
	def obtenerActivos(params){
		...
		def estatus = EstatusCertificado.createCriteria().list {
			eq("activo", true)
		}
	...
	def consultar(params) {
		...
		def estatus = EstatusCertificado.get(params.id)
