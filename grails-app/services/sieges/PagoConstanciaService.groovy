...
class PagoConstanciaService {
    ...
    def registrar(params){
        ...
        def datosBitacora = [
            clase: "pagoConstanciaService",
            metodo: "registrar",
            nombre: "Registro de tramite de constancia",
        ...
        def resultadoOperacion = pagoService.consultarPago(params, tipoTramiteService.CONSTANCIA)
        ...
        if(!params.constancias){
            resultado.mensaje = 'Las constancias son un dato requerido'
            return resultado
        }
        ...
        def uma = Uma.get(1)
        def tipoTramite = tipoTramiteService.obtener(tipoTramiteService.CONSTANCIA)
        def precioConstancia = tipoTramite.costoUmas * uma.valor

        def numConstanciasPagados = Math.floor(pago.importe / precioConstancia)

        def constanciasId = []
        try{

            def jsonSlurper = new JsonSlurper()
            constanciasId = jsonSlurper.parseText(params.constancias)

            if(!constanciasId){
                resultado.mensaje = 'Los constancia son un dato requerido'
                return resultado
            }

            if(constanciasId.size > numConstanciasPagados){
                resultado.mensaje = "Seleccione ${numConstanciasPagados} constancia"
                return resultado
            }

            for(constanciaId in constanciasId){
                def constancia = ConstanciaServicio.get(constanciaId)
                if(!constancia){
                    resultado.mensaje = "No se encontró el constancia"
                    return resultado
                }
                if(!constancia.activo){
                    resultado.mensaje = "El constancia no se encuentra activo"
                    return resultado
                }
                if(constancia.tramite != null){
                    resultado.mensaje = "El constancia ya cuenta con un tramite asignado"
                    return resultado
                }
                constancia.alumno.ciclosEscolares.each{ ciclo ->
		...
		constanciasId.each{ constanciaId ->
			def constancia = ConstanciaServicio.get(constanciaId)
			constancia.tramite = tramite
			constancia.estatusConstancia = EstatusConstancia.get(constanciaService.EN_REVISION)
            constancia.save()
		}
        ...
        Pago.withTransaction {status ->
            if(tramite.pago.save(flush: true)){
                if(tramite.save(flush: true)){
                    resultado.estatus = true
                    bitacoraService.registrar([clase: "PagoConstanciaService", metodo:"modificar", nombre:"Modificacion de tramite de certificación", descripcion:"Se modifica el tramite", estatus: "EXITOSO"])
                }else{
                    status.setRollbackOnly()
                    tramite.errors.allErrors.each {
                        resultado.mensaje = messageSource.getMessage(it, null)
                    }
                    bitacoraService.registrar([clase: "PagoConstanciaService", metodo: "modificar", nombre:"Modificacion de tramite de certificación", descripcion: "Se modifica el tramite", estatus:"ERROR"])
                }
            }
        }
    ...
    def eliminar(params){
        ...
        if(!tramite.save(flush:true)){
            ...
            bitacoraService.registrar([clase:"PagoConstanciaService", metodo: "eliminar", nombre:"Eliminación del tramite", descripcion:"Se elimina el tramite", estatus:"EXITOSO"])
            return resultado
        }
        bitacoraService([clase:"PagoConstanciaService", metodo:"eliminar", nombre:"Eliminación del tramite", descripcion:"Se elimina el tramite", estatus:"ERROR"])
