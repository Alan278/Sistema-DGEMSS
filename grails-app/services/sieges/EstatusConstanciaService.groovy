...
@Transactional
class EstatusConstanciaService {
/**
	...
	def listar(params){
		...
		def estatus = EstatusConstancia.createCriteria().list(params,criteria)
	...
	def obtenerActivos(params){
		...
		def estatus = EstatusConstancia.createCriteria().list {
			eq("activo", true)
		}
	...
	def consultar(params) {
		...
		def estatus = EstatusConstancia.get(params.id)
